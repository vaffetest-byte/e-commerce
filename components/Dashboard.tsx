    ...
    <div className="space-y-16 animate-in fade-in duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 border-b border-slate-100 pb-16">
        <div>
          ...
          <h1 className="text-6xl font-black tracking-tighter text-[#0A0A0A] serif italic">Editorial <span className="not-italic font-black">Stats</span></h1>
        </div>
        <div className="flex items-center gap-6 bg-white p-3 rounded-[32px] border border-slate-100 shadow-sm">
            ...
            <button className="px-8 py-3 text-[10px] font-black uppercase tracking-[0.3em] text-black/20 hover:text-black transition-colors">Refresh Analysis</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                { label: 'Conversion', val: '3.4%', trend: '+0.2%', icon: Target, color: 'text-amber-600', bg: 'bg-amber-50' },
                { label: 'Fulfillment', val: '98%', trend: 'Steady', icon: Layers, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                ].map((s, i) => (
                <div key={i} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm hover:shadow-xl transition-all group cursor-default">
                    <div className={`w-12 h-12 ${s.bg} ${s.color} rounded-[18px] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}>
                        <s.icon size={20} strokeWidth={2} />
                    </div>
                    <div className="flex justify-between items-baseline mb-1">
                        <p className="text-[9px] font-black uppercase tracking-[0.4em] text-black/30">{s.label}</p>
                        <span className="text-[8px] font-black text-emerald-500">{s.trend}</span>
                    </div>
                    <h3 className="text-3xl font-black text-[#0A0A0A] tracking-tighter">{s.val}</h3>
                </div>
                ))}
            </div>

            {/* AI Intelligence Block */}
            <div className="relative rounded-[60px] bg-[#0A0A0A] p-16 overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row gap-12 items-start">
                    <div className="shrink-0">
                        <div className="w-20 h-20 rounded-[30px] bg-rose-600 flex items-center justify-center text-white shadow-[0_20px_40px_-10px_rgba(225,29,72,0.4)] group-hover:rotate-12 transition-transform duration-700">
                            <Sparkles size={32} strokeWidth={1} />
                        </div>
                    </div>
                    <div className="flex-1">
                        <div className="flex items-center gap-6 mb-6">
                            <div className="h-[1px] w-10 bg-rose-600" />
                        ...
                        <div className="max-w-3xl">
                            {loadingInsight ? (
                                ...
                                <div className="text-white text-xl md:text-2xl font-light serif italic leading-relaxed tracking-tight whitespace-pre-line">
                                    {insight}
                                </div>
                            )}
                        </div>
        ...
        <div className="lg:col-span-4 space-y-10">
            {/* Recent Activity */}
            <div className="bg-white p-10 rounded-[50px] border border-slate-100 shadow-sm flex flex-col h-full">
                ...
                <div className="space-y-8 flex-1">
                    {recentActivities.map((act, i) => (
                        <div key={i} className="flex gap-6 group">
                            <div className={`w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center ${act.color} group-hover:bg-white group-hover:shadow-md transition-all`}>
                                <act.icon size={16} strokeWidth={2.5} />
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between items-baseline mb-1">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-[#0A0A0A]">{act.label}</span>
                                    <span className="text-[8px] font-bold text-slate-300">{act.time}</span>
                                </div>
                                <p className="text-[10px] text-slate-400 font-medium leading-none">{act.detail}</p>
                            </div>
                        </div>
                    ))}
                </div>
                <button className="w-full py-4 border border-slate-100 rounded-2xl text-[9px] font-black uppercase tracking-[0.3em] mt-10 hover:bg-slate-50 transition-all flex items-center justify-center gap-3">
                    View Full Registry <ArrowRight size={12} />
                </button>
            </div>

            {/* Critical Inventory Alerts */}
            <div className="bg-rose-50/50 p-10 rounded-[50px] border border-rose-100/50 flex flex-col">
                ...
                <div className="space-y-4">
                    {lowStockItems.slice(0, 3).map((item, i) => (
                        <div key={i} className="bg-white p-5 rounded-3xl border border-rose-100 flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black text-rose-950 uppercase tracking-tight">{item.name}</p>
                ...
                <button className="mt-8 text-[9px] font-black uppercase tracking-[0.3em] text-rose-500 text-center hover:underline">Manage All Inventory</button>
            </div>
        </div>
      </div>
    </div>
